...
public abstract class WebHelpMojo extends AbstractWebhelpMojo {
	...
	targetFile = new File(new File(getTargetDirectory(), warBasename), warBasename + ".xml");
   ...
   transformer.setParameter("compute.wadl.path.from.docbook.path",computeWadlPathFromDocbookPath);
	...
	result = new File(new File( getTargetDirectory(), warBasename), warBasename + ".xml" );
	...
	if(null != webhelpWar && !"0".equals(webhelpWar)){
	    ...
	    File zipFile = new File(new File(new File(result.getParentFile().getParentFile(), properties.getProperty("warprefix","")), warBasename), warSuffixForWar + ".war");
	    //result.deleteOnExit();
	...
	// String targetDirFiguresString = baseDir.getAbsolutePath()+File.separator+"figures";
        // map.put("targetDir", targetDirFiguresString.replace(File.separatorChar, '/'));
